...
public class Config : SyncedConfig2<Config>
{
    ...
    [SyncedEntryField] public SyncedEntry<float> CreditPenaltyPerPlayer;
    [SyncedEntryField] public SyncedEntry<bool> CreditPenaltiesDynamic;
    [SyncedEntryField] public SyncedEntry<float> CreditPenaltyCap;
    [SyncedEntryField] public SyncedEntry<float> CreditPenaltyThreshold;
    ...
    [SyncedEntryField] public SyncedEntry<float> QuotaPenaltyPerPlayer;
    [SyncedEntryField] public SyncedEntry<bool> QuotaPenaltiesDynamic;
    [SyncedEntryField] public SyncedEntry<float> QuotaPenaltyCap;
    [SyncedEntryField] public SyncedEntry<float> QuotaPenaltyThreshold;
    ...
    [SyncedEntryField] public SyncedEntry<float> MaxFractionLostScrapItems;

    [SyncedEntryField] public SyncedEntry<bool> ValueLossEnabled;
    [SyncedEntryField] public SyncedEntry<float> ValueLossAmount;
    ...
    [SyncedEntryField] public SyncedEntry<float> MaxFractionLostEquipmentItems;
    ...
    public Config(ConfigFile config) : base(Plugin.PluginGuid)
    {
        ...
        CreditPenaltiesEnabled = config.BindSyncedEntry("Credit Penalties", "Enable", false, "When enabled, you will lose a fraction of your credits for every crew memeber that dies.  You can also enable dynamic mode, in which case the penalty is based on the ratio of dead players to alive players. \nVanilla: true");
        ...
        CreditPenaltyPerPlayer = config.BindSyncedEntry("Credit Penalties", "Penalty Per Player", 0.2f, "The fraction of your credits that you lose for each dead player.  Example:  Penalty Per Player = 20%, you have 1000 credits, 2 players die, you lose 400. \nValues >= 0");
        ...
        CreditPenaltyCap = config.BindSyncedEntry("Credit Penalties", "Dynamic Penalty Cap", 0.8f, "The penalty in the worst case scenario, all players dead and unrecovered. Any players still alive, and any bodies recovered (see Body Recovery Bonus) will reduce the penalty. \nValues >= 0");
        CreditPenaltyThreshold = config.BindSyncedEntry("Credit Penalties", "Dynamic Penalty Threshold", 0.2f, "If the penalty falls below this threshold, the penalty is set to 0. Increasing this value makes minor slip-ups more forgiving.  This applies to both the static and dynamic algorithms. \nValues > 0");
        CreditPenaltyRecoveryBonus = config.BindSyncedEntry("Credit Penalties", "Body Recovery Bonus", 0.2f, "How much of the penalty to forgive for recovering bodies. A higher value means a higher incentive to recover bodies.  Applies to both normal and dynamic modes. \nValues between 0-100");
        CreditPenaltyOnTeamWipe = config.BindSyncedEntry("Credit Penalties", "Team Wipe Penalty", 0f, "The penalty that will be applied in the case of all players being dead.  A value <= 0 disables this feature.  This applies to either flat or dynamic mode.  Recovered bodies still reduce the penalty as normal.");
        ...
        QuotaPenaltyPerPlayer = config.BindSyncedEntry("Quota Penalties", "Penalty Per Player", 0.12f, "The amount to increase the quota per dead player, as a multiple of the quota for this round. \nValues >= 0");
        ...
        QuotaPenaltyCap = config.BindSyncedEntry("Quota Penalties", "Dynamic Penalty Cap", 0.5f, "The penalty in the worst case scenario, all players dead and unrecovered. Any players still alive, and any bodies recovered (see Body Recovery Bonus) will reduce the penalty. \nValues >= 0");
        QuotaPenaltyThreshold = config.BindSyncedEntry("Quota Penalties", "Dynamic Penalty Threshold", 0.25f, "If the penalty falls below this threshold, the penalty is set to 0. Increasing this value makes minor slip-ups more forgiving.  This applies to both the static and dynamic algorithms. \nValues between 0-100");
        QuotaPenaltyRecoveryBonus = config.BindSyncedEntry("Quota Penalties", "Body Recovery Bonus", 0.5f, "How much of the penalty to forgive for recovering bodies.  Applies to both normal and dynamic modes.  For example:  Assuming a fully default coniguration, except without Dynamic Penalties, if you die, the penalty for your body is 0.12.  If your body is recovered, 0.5x of the penalty is forgiven, leaving a penalty of 0.06. \nValues > 0");
        QuotaPenaltyOnTeamWipe = config.BindSyncedEntry("Quota Penalties", "Team Wipe Penalty", 0f, "The penalty that will be applied in the case of all players being dead.  A value <= 0 disables this feature.  This applies to either flat or dynamic mode.  Recovered bodies still reduce the penalty as normal.");
        ...
        ItemsSafeChance = config.BindSyncedEntry("Scrap Loss", "Safe Chance", 0.25f, "A chance between 0 and 1 of all scrap and equipment being safe. When your items are 'safe', it overrides all other settings, and you keep everything. \nValues between 0-1 \nVanilla: 0");
        ValueLossEnabled = config.BindSyncedEntry("Scrap Loss", "Value Loss Enabled", true, "If enabled, you lose a fraction of the total scrap value on board on a crew wipe. \nVanilla: false.");
        ValueLossAmount = config.BindSyncedEntry("Scrap Loss", "Value Loss Amount", 1f, "The fraction of total scrap value to lose. \nValues between 0-1");
        LoseEachScrapChance = config.BindSyncedEntry("Scrap Loss", "Lose Each Chance", 0.5f, "A chance between 0 and 1 of each item being lost. \nValues between 0-1 \nVanilla: 0");
        ...
        MaxFractionLostScrapItems = config.BindSyncedEntry("Scrap Loss", "Lost Scrap Max Fraction", 1f, "The maximum fraction of scrap items that can be lost.  For example, if this is set to 0.5, and you have 8 scrap on the ship, you cannot lose more than 4 scrap.");

        EquipmentLossEnabled = config.BindSyncedEntry("Equipment Loss", "Equipment Loss Enabled", false, "Allow equipment to be lost. \nVanilla: false.");
        LoseEachEquipmentChance = config.BindSyncedEntry("Equipment Loss", "Equipment Loss Chance", 0.1f, "A chance of each equipment item being lost. \nApplied after SaveAllChance. \nValues between 0-1 \nVanilla: 0");
        ...
        MaxFractionLostEquipmentItems = config.BindSyncedEntry("Equipment Loss", "Lost Equipment Max Fraction", 1f, "The maximum fraction of equipment items that can be lost.  For example, if this is set to 0.5, and you have 6 equipment items on the ship, you cannot lose more than 3 equipment items. \nValues between 0-1");
